...
public class FaceDetectionImage {
	...
	private ArrayList<ArrayList<code.Point>> points = new ArrayList<ArrayList<code.Point>>();
	...
	public WritableImage process () {
		...
		File opencvLib = new File("src/libs/libopencv_java452.so");
		...
		for (Rect rect : faceDetections.toArray()) {
			...
			if(intersection) {
				...
				ArrayList<code.Point> pts = new ArrayList<code.Point>();
				pts.add(new code.Point((int) (rect.x + (rect.width *0.15))  ,(int) (rect.y + (rect.height * 0.15))));
				pts.add(new code.Point((int) ((rect.x + rect.width) - (0.15*(rect.width))) , (int) (( rect.y + rect.height) - (rect.height * 0.15))));
				points.add(pts);
			...
			// If there is no intersection -> standard selection
				...
				// Adding coords to a liste
				ArrayList<code.Point> pts = new ArrayList<code.Point>();
				pts.add(new code.Point(rect.x, rect.y));
				pts.add(new code.Point(rect.x + rect.width, rect.y + rect.height));
				points.add(pts);
	...
	public ArrayList<ArrayList<code.Point>> getPoints() {
		
		ArrayList<ArrayList<code.Point>> orderedPoints= points;
		
		Comparator<ArrayList<code.Point>> PointXComparator = new Comparator<ArrayList<code.Point>>()
	    {
	        public int compare(ArrayList<code.Point> p1, ArrayList<code.Point> p2)
	        {
	...
	public void printPoints() {
		for (ArrayList<code.Point> arrayList : points) {
			System.out.println("[");
			for (code.Point pts : arrayList) {
